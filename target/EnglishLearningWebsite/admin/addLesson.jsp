...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/admin/css/admin-style.css"> <%-- Sử dụng lại CSS nếu có --%>
            ...
            <main role="main" class="col-md-10 ml-sm-auto admin-main-content">
                ...
                <c:if test="${not empty requestScope.errorMessage_addLesson}">
                    <div class="alert alert-danger" role="alert">
                        <c:out value="${requestScope.errorMessage_addLesson}"/>
                    </div>
                </c:if>

                <form method="POST" action="${pageContext.request.contextPath}/admin/add-lesson-action">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-lg" id="lessonTitle" name="lessonTitle" required 
                               value="<c:out value='${param.lessonTitle}'/>"> <%-- Giữ lại giá trị đã nhập nếu có lỗi --%>
                    </div>
                    <div class="form-group">
                        <label for="lessonContent"><strong>Nội dung bài học:</strong></label>
                        <textarea class="form-control form-control-lg" id="lessonContent" name="lessonContent" rows="10" required><c:out value='${param.lessonContent}'/></textarea> <%-- Giữ lại giá trị đã nhập nếu có lỗi --%>
                    </div>
                    <button type="submit" class="btn btn-primary btn-lg">Thêm Bài Học</button>
